...
function getImageUrl(ilan: any): string {
  ...
  if (img && typeof img === "string" && img.length > 5) return img;
...
// ─── BorsaKarti — component dışında tanımlandı (re-render önlendi) ────────
...
const BorsaKarti = memo(function BorsaKarti({
  ...
  // İlk 4 kart eager — LCP için kritik
      ...
      <div className="card-body">
        ...
        <div className="card-actions">
          ...
          <div className="action-row-bottom">
            <button onClick={() => onOpenModal(ilan, "takas")} className="btn-swap">
              🔄 Takas Teklifi
            </button>
            <button onClick={() => onOpenModal(ilan, "satinal")} className="btn-buy">
              Satın Al
            </button>
          </div>
        </div>
...
const MARKETPLACE_LD = {
  "@context": "https://schema.org",
  "@type": "ItemList",
  name: "A-TAKASA Vitrin İlanları",
  description: "Türkiye genelinde satılık, takaslanabilir ürünler",
  url: "https://atakasa.com",
};

// ─── Sabit diziler — dışarıda tanımlandı (her render'da yeniden oluşmaz) ─
const SEHIRLER = [
...
export default function Home() {
  ...
  const router = useRouter();
  const ilkPreloadEklendi = useRef(false);
  ...
  // Veri çekme — ilk 20 hızlı, geri kalan lazy
  useEffect(() => {
    const veriCek = async () => {
      try {
        const res = await fetch("/api/varliklar?limit=20&durum=aktif");
        const data = await res.json();
        const liste: any[] = Array.isArray(data)
          ? data
          : data.data || data.ilanlar || data.varliklar || [];
        setIlanlar(liste);
        setLoading(false);

        // ✅ LCP: İlk görsel preload — bir kez eklenir
        if (!ilkPreloadEklendi.current && liste.length > 0) {
          ilkPreloadEklendi.current = true;
          const ilkImg = getImageUrl(liste[0]);
          if (ilkImg && !isVideoUrl(ilkImg)) {
            const link = document.createElement("link");
            link.rel = "preload";
            link.as = "image";
            link.href = optimizeCloudinary(ilkImg, 520, 220);
            link.setAttribute("fetchpriority", "high");
            document.head.appendChild(link);
          }
        }

        // Geri kalan ilanları arka planda yükle
        if (liste.length === 20) {
          setTimeout(async () => {
            try {
    ...
    if (session?.user?.email && ilanlar.length > 0) {
      ...
      setBenimIlanlarim(
        ilanlar.filter((i: any) => {
          const sEmail = (i.satici?.email || i.sellerEmail || i.satici || "")
  ...
  const filtrelenmisIlanlar = useMemo(() => {
    ...
    if (aktifSehir !== "Tüm Şehirler")
      liste = liste.filter((i) =>
        (i.sehir || "").toUpperCase() === aktifSehir.toUpperCase()
      );
    if (minFiyat)
  ...
  return (
    <div className="at-root">
      ...
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(MARKETPLACE_LD) }}
      />
      ...
      <nav className="top-nav" aria-label="Ana navigasyon">
        <div className="nav-inner">
          <div
            onClick={() => router.push("/")}
            style={{ cursor: "pointer", marginRight: 16, display: "flex", alignItems: "center" }}
            role="link"
            aria-label="Ana sayfaya git"
          >
            <h1 style={{
              color: "var(--navy)", fontSize: 24, fontWeight: 800,
              ...
              <div className="modal-form">
                ...
                <div className="legal-box">
                  <label className="legal-check">
                    <input type="checkbox" checked={kabulSozlesme}
