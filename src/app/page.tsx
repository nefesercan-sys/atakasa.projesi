...
export default function Home() {
  ...
  const filtrelenmisIlanlar = () => {
    ...
    if (aktifKategori === "Hepsi") {
      // 📱 KATEGORİ YOKSA: Instagram Usulü Karışık & Yeniden Eskiye Akış
      liste.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    } else {
      // 🎯 KATEGORİ SEÇİLDİYSE: Sadece o kategori ve Seçilen Alt Filtreye Göre
      ...
      switch (aktifAltFiltre) {
        ...
        default: liste.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()); // Yeni İlanlar
      }
    }
    return liste;
  };
  ...
  const handleSepeteEkle = (ilan: any) => {
    // Şimdilik LocalStorage tabanlı siber sepet. (Backend'e bağlanabilir)
  ...
  // 🚀 TAKAS API GÖNDERİMİ (Panele Yansıyacak Format)
    ...
    try {
      const res = await fetch("/api/takas", {
        ...
        body: JSON.stringify({ 
          ...
          durum: "bekliyor" // Panele düşmesi için ilk statü
        })
      });
      if (res.ok) { alert("⚡ TAKAS TEKLİFİ BAŞARIYLA İLETİLDİ! (Panelinizden takip edebilirsiniz)"); closeModal(); }
      else { const err = await res.json(); alert(`İhlal: ${err.error}`); }
    } catch (e) { alert("Sinyal koptu."); }
  };

  // 🛒 SATIN ALMA API GÖNDERİMİ (Panele Yansıyacak Format)
  const handleSiparisTamamla = async () => {
    ...
    try {
      ...
      if (res.ok) { alert("📦 SİPARİŞ ONAYLANDI! (Panelinizden kargo durumunu takip edebilirsiniz)"); closeModal(); }
      else { alert("Sipariş ağına ulaşılamadı."); }
    } catch (error) { alert("Bağlantı hatası."); }
  };

  // 🃏 PROFESYONEL SİBER VARLIK KARTI (INSTAGRAM AKIŞ MODELİ)
  const BorsaKarti = ({ ilan }: { ilan: any }) => (
    <div className="bg-[#0a0a0a] border border-white/5 rounded-[2.5rem] overflow-hidden hover:border-[#00f260]/40 transition-all group shadow-2xl flex flex-col h-full relative">
      ...
      {/* 📉 YÜZDESEL FİYAT DEĞİŞİMİ KÖŞE ROZETİ */}
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans italic pb-24 selection:bg-[#00f260] selection:text-black">
      ...
      {/* 🚀 KONTROL PANELİ (ARAMA, KATEGORİLER, ALT FİLTRELER) */}
      <div className="sticky top-0 z-[100] bg-[#050505]/95 backdrop-blur-3xl border-b border-white/5 pt-8 pb-4 px-4 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="relative mb-5 group">
            ...
            <input 
              type="text" placeholder="Borsada varlık ara (iPhone, Arsa, Saat)..." 
              className="w-full bg-[#0a0a0a] border border-white/10 rounded-[2rem] px-6 py-4 outline-none focus:border-[#00f260] text-sm transition-all"
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2 mask-linear-right">
            <button 
              ...
              className={`px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${aktifKategori === "Hepsi" ? 'bg-[#00f260] text-black shadow-[0_0_15px_rgba(0,242,96,0.3)]' : 'bg-white/5 text-slate-400 border border-white/5 hover:bg-white/10'}`}
            >
              🌐 KARIŞIK AKIŞ
            </button>
            {sektorler.map(s => (
              <button 
                ...
                className={`px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${aktifKategori === s.ad ? 'bg-white text-black shadow-[0_0_15px_rgba(255,255,255,0.3)]' : 'bg-white/5 text-slate-400 border border-white/5 hover:bg-white/10'}`}
              >
                {s.ad} <span className="opacity-50 text-[8px] pl-1">{s.degisim}%</span>
              </button>
            ))}
          </div>

          {/* 3. Alt Filtreler (Sadece Kategori Seçilince Görünür) */}
          {aktifKategori !== "Hepsi" && (
            <div className="flex gap-2 mt-3 animate-in slide-in-from-top duration-300 overflow-x-auto no-scrollbar">
              {["Yeni İlanlar", "En Çok Fiyatı Düşenler", "En Çok Yükselenler", "En Çok Takas Edilenler"].map(f => (
                <button 
                  ...
                  className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${aktifAltFiltre === f ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'text-slate-500 bg-transparent hover:text-white'}`}
                >
                  {f}
                </button>
              ))}
      ...
      {/* 📸 INSTAGRAM TARZI SONSUZ AKIŞ VİTRİNİ */}
      <main className="max-w-7xl mx-auto px-4 mt-8 relative z-10">
        
        {/* Dinamik Slogan ve Başlık */}
        <div className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-white/5 pb-6">
          <div>
            ...
            <p className="text-slate-500 text-xs md:text-sm font-bold tracking-widest uppercase">
              {aktifKategori === "Hepsi" ? "Tüm siber ağ karışık listeleniyor." : `Şu an ${aktifKategori} sektöründeki ${aktifAltFiltre.toLowerCase()} listeleniyor.`}
            </p>
          </div>
          <div className="bg-[#00f260]/10 border border-[#00f260]/20 px-6 py-3 rounded-2xl text-[#00f260] text-[10px] font-black tracking-[0.3em] uppercase animate-pulse">
            {aktifSlogan}
          </div>
        </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-black/40 border border-white/5 p-4 rounded-xl space-y-3 mt-4">
                  <label className="flex items-start gap-3 cursor-pointer group">
                    <input type="checkbox" checked={kabulSozlesme} onChange={(e) => setKabulSozlesme(e.target.checked)} className="mt-1 accent-[#00f260] w-4 h-4" />
                  ...
                  <label className="flex items-start gap-3 cursor-pointer group">
                    <input type="checkbox" checked={kabulYasalZirh} onChange={(e) => setKabulYasalZirh(e.target.checked)} className="mt-1 accent-[#00f260] w-4 h-4" />
