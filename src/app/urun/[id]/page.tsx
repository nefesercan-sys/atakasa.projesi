...
import { ShieldCheck, ArrowLeftRight, ShoppingCart, Zap, MessageCircle } from "lucide-react";
...
export default function SiberVarlikTerminali({ params }: { params: any }) {
  ...
  if (loading) return <div className="min-h-screen bg-[#020202] flex flex-col items-center justify-center text-[#00f260] font-black tracking-widest animate-pulse"><Zap size={40} className="mb-4" /> VARLIK ÇÖZÜMLENİYOR...</div>;
  if (!ilan) return <div className="min-h-screen bg-[#020202] flex items-center justify-center text-red-500 font-black tracking-widest">VARLIK BULUNAMADI.</div>;
      ...
      <div className="max-w-6xl mx-auto bg-[#0a0a0a] border border-white/5 rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col lg:flex-row relative z-10">
        ...
        <div className="w-full lg:w-1/2 bg-[#030712] border-r border-white/5 p-6 md:p-8 flex flex-col relative">
           ...
           <div className="mt-auto bg-white/5 p-6 rounded-3xl border border-white/5 max-h-96 overflow-y-auto scrollbar-hide">
             <div className="flex justify-between items-center mb-6 border-b border-white/5 pb-4">
               <h3 className="text-cyan-400 font-black text-[10px] uppercase tracking-widest flex items-center gap-2">
                 Satıcı Güven Puanı
               </h3>
        ...
        <div className="w-full lg:w-1/2 p-6 md:p-8 flex flex-col">
           ...
           <div className="flex bg-[#000000] p-1.5 rounded-2xl mb-8 border border-white/5 shadow-inner">
             <button onClick={()=>setAktifSekme("incele")} className={`flex-1 py-4 rounded-xl font-black text-[9px] md:text-[10px] uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${aktifSekme === "incele" ? 'bg-white/10 text-white' : 'text-slate-500 hover:text-white'}`}>
               🔍 İncele
             </button>
             {!ilaninSahibiyim && (
               <>
                 <button onClick={()=>setAktifSekme("takas")} className={`flex-1 py-4 rounded-xl font-black text-[9px] md:text-[10px] uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${aktifSekme === "takas" ? 'bg-white/10 text-white' : 'text-slate-500 hover:text-white'}`}>
                   TAKAS YAP
                 </button>
                 <button onClick={()=>setAktifSekme("satinal")} className={`flex-1 py-4 rounded-xl font-black text-[9px] md:text-[10px] uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${aktifSekme === "satinal" ? 'bg-[#00f260] text-black shadow-[0_0_15px_rgba(0,242,96,0.3)]' : 'text-slate-500 hover:text-white'}`}>
                   SATIN AL
                 </button>
               </>
             )}
             ...
             <div className="flex flex-col h-full animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                {!ilaninSahibiyim && (
                  <div className="flex flex-col gap-4 mt-auto">
                    <Link href={`/mesajlar?satici=${ilan.sellerEmail || ilan.satici?.email || ilan.userId}&ilan=${ilan._id || ilan.id}`} className="w-full bg-white/5 text-white py-5 rounded-2xl border border-white/10 font-black uppercase tracking-widest text-[11px] hover:bg-white/10 transition-all flex justify-center items-center gap-2">
                ...
                <div className="space-y-4 mb-8 mt-auto border border-white/5 bg-[#030712] p-5 rounded-2xl">
                  <label className="flex items-start gap-3 cursor-pointer group">
                    ...
                    <p className="text-[9px] text-zinc-500 leading-relaxed uppercase font-black tracking-widest group-hover:text-zinc-300 transition-colors">
                      MESAFELİ SATIŞ SÖZLEŞMESİ'Nİ OKUDUM, ANLADIM VE KABUL EDİYORUM. TÜM SORUMLULUK TARAFIMA AİTTİR.
                    </p>
                  </label>
                  <label className="flex items-start gap-3 cursor-pointer group">
                    <div className="relative flex items-center justify-center mt-0.5 shrink-0">
                      <input type="checkbox" checked={true} readOnly className="appearance-none w-5 h-5 border-2 border-white bg-white rounded-md transition-colors" />
                    ...
                    <p className="text-[9px] text-zinc-500 leading-relaxed uppercase font-black tracking-widest">
                      <span className="text-[#00f260]">🛡️ YASAL ZIRH:</span> BU İŞLEM SİBER KALKAN GÜVENCESİNDEDİR. TESLİMAT TAMAMLANANA KADAR PARA HAVUZA AKTARILIR. ONAYLIYORUM.
                    </p>
                  </label>
                </div>

                <button onClick={handleSiparisTamamla} className="w-full bg-[#00f260] text-black py-4 rounded-xl font-black uppercase tracking-[0.1em] text-[11px] hover:scale-[1.02] transition-all shadow-[0_0_20px_rgba(0,242,96,0.2)] flex items-center justify-center gap-2">
