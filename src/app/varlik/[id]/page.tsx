        ...
        <div className="w-full lg:w-1/2 bg-[#030712] border-r border-white/5 p-6 md:p-8 flex flex-col relative">
          ...
          >
            {aktifMedyaVideo ? (
              ...
              <img
                src={aktifMedya}
                alt={ilan.baslik}
                className="w-full h-auto max-h-[450px] object-cover opacity-90 group-hover:scale-105 transition-transform duration-700"
              />
            )}
          </div>
              ...
              {tumMedya.map((medya, idx) => (
                <div
                  ...
                  className={`relative shrink-0 w-16 h-16 rounded-xl overflow-hidden cursor-pointer border-2 transition-all ${
                    aktifMedyaIndex === idx ? 'border-[#00f260] shadow-[0_0_10px_rgba(0,242,96,0.4)]' : 'border-white/10 hover:border-white/30'
                  }`}
                >
                  {isVideo(medya) ? (
          ...
          <div className="mt-auto bg-white/5 p-6 rounded-3xl border border-white/5 max-h-96 overflow-y-auto">
            ...
            <div className="space-y-4 mb-6">
              {yorumlar.length === 0 ? (
                <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest text-center py-4 bg-black/40 rounded-xl">
        ...
        <div className="w-full lg:w-1/2 p-6 md:p-8 flex flex-col">
          ...
          {aktifSekme === "incele" && (
            <div className="flex flex-col h-full">
              ...
              {!ilaninSahibiyim && (
                <Link
                  href={`/mesajlar?satici=${ilan.satici?.email || ilan.sellerEmail || ilan.userId}&ilan=${ilan._id}`}
            ...
            <div className="flex flex-col h-full">
              ...
              {benimIlanlarim.length === 0 && (
                <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-4 mb-4 text-center">
                  ...
                  <button onClick={() => router.push('/varlik-ekle')} className="mt-2 text-[#00f260] text-[10px] font-black uppercase underline">
                    İlan Ver →
                  </button>
                </div>
              )}
              ...
              <input
                type="number" placeholder="Örn: 500" value={eklenecekNakit}
              ...
              <textarea
                value={takasMesaji} onChange={(e) => setTakasMesaji(e.target.value)}
      ...
      {videoAcik && aktifMedyaVideo && (
        ...
        >
          <div className="relative w-full max-w-3xl" onClick={e => e.stopPropagation()}>
            <button
              onClick={() => setVideoAcik(false)}
              className="absolute -top-14 right-0 text-white bg-white/10 hover:bg-red-500 w-10 h-10 rounded-full flex items-center justify-center font-black text-lg transition-all z-10"
            >✕</button>
            <video
              src={aktifMedya}
              controls autoPlay
