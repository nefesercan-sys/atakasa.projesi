...
import { Share2, ShoppingCart, ArrowLeft, MapPin, Calendar, Shield, MessageCircle } from "lucide-react";
  ...
  useEffect(() => {
    ...
    fetch(`/api/varliklar/${id}`)
      ...
      .then((data) => {
        ...
        setIlan(d);
        setLoading(false);
        if (d) document.title = `${d.baslik} | ${d.sehir || "Türkiye"} — A-TAKASA`;
      })
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ textAlign: "center" }}>
        ...
        <p style={{ color: "#8097b1", fontSize: 14, fontFamily: "sans-serif" }}>Yükleniyor...</p>
    ...
    <div style={{ minHeight: "100vh", background: "#f0f2f5", fontFamily: "'DM Sans', sans-serif", paddingBottom: 100 }}>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #e8edf3", padding: "12px 20px", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <nav style={{ fontSize: 12, color: "#8097b1", display: "flex", gap: 6, alignItems: "center" }}>
          ...
          <span>›</span>
          <a href={`/kategori/${(ilan.kategori || "diger").toLowerCase().replace(/\s/g, "-")}`} style={{ color: "#8097b1", textDecoration: "none" }}>{ilan.kategori || "İlanlar"}</a>
          <span>›</span>
          <span style={{ color: "#0f2540", fontWeight: 600 }}>{ilan.baslik}</span>
        </nav>
      </div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "24px 16px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
          ...
          <div>
            <div style={{ background: "#fff", borderRadius: 20, overflow: "hidden", border: "1px solid #e8edf3", aspectRatio: "4/3" }}>
            ...
            {gorseller.length > 1 && (
              <div style={{ display: "flex", gap: 8, marginTop: 12, overflowX: "auto" }}>
                {gorseller.map((g, i) => (
                  <div key={i} onClick={() => setAktifGorsel(i)} style={{ width: 72, height: 72, borderRadius: 10, overflow: "hidden", border: `2px solid ${aktifGorsel === i ? "#0f2540" : "#e8edf3"}`, cursor: "pointer", flexShrink: 0 }}>
                    {isVideo(g) ? (
                      <video src={g} muted style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                    ) : (
                      <img src={optimizeCloudinary(g, 144, 144)} alt="" width={72} height={72} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                    )}
                  </div>
                ))}
              </div>
          ...
          <div>
            <div style={{ background: "#fff", borderRadius: 20, padding: 28, border: "1px solid #e8edf3" }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: "#c9a84c", textTransform: "uppercase", letterSpacing: "0.07em" }}>{ilan.kategori || "Genel"}</span>
              <h1 style={{ fontSize: 22, fontWeight: 800, color: "#0f2540", marginTop: 8, marginBottom: 16, lineHeight: 1.3 }}>{ilan.baslik}</h1>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 24 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "#5a6a7e" }}>
                  <Shield size={15} style={{ color: "#22c55e" }} /> Siber Kalkan Korumalı
                </div>
              </div>
        ...
        <div style={{ background: "linear-gradient(135deg, #f0f9ff, #e0f2fe)", borderRadius: 20, padding: 24, border: "1px solid #bae6fd", marginTop: 20 }}>
          ...
          <p style={{ fontSize: 13, color: "#0369a1", lineHeight: 1.7 }}>
            Ödemeniz, ürün teslim edildiğini onaylayana kadar güvenli havuzda bekler. Takas anlaşmazlıklarında platformumuz arabuluculuk yapar.
          </p>
        </div>
      </div>
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: 20, width: "100%", maxWidth: 480, maxHeight: "90vh", overflowY: "auto", padding: 28 }}>
            ...
            {modalTuru === "takas" ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <input type="number" placeholder="Üste nakit ekle (₺) — isteğe bağlı" value={eklenecekNakit}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <input type="text" placeholder="Ad Soyad" autoComplete="name" value={siparisForm.adSoyad}
                  onChange={(e) => setSiparisForm({ ...siparisForm, adSoyad: e.target.value })}
                  style={{ padding: "12px 14px", background: "#f5f7fa", border: "1.5px solid #e0e8f0", borderRadius: 10, fontFamily: "inherit", fontSize: 13, outline: "none" }} />
                <input type="tel" placeholder="Telefon" autoComplete="tel" value={siparisForm.telefon}
                  onChange={(e) => setSiparisForm({ ...siparisForm, telefon: e.target.value })}
                  style={{ padding: "12px 14px", background: "#f5f7fa", border: "1.5px solid #e0e8f0", borderRadius: 10, fontFamily: "inherit", fontSize: 13, outline: "none" }} />
                <textarea placeholder="Teslimat Adresi" rows={3} value={siparisForm.adres}
                ...
                <div style={{ background: "#f8fafc", padding: 14, borderRadius: 12, display: "flex", flexDirection: "column", gap: 10 }}>
                  <label style={{ display: "flex", alignItems: "flex-start", gap: 10, cursor: "pointer", fontSize: 12, color: "#5a6a7e" }}>
                    <input type="checkbox" checked={kabulSozlesme} onChange={(e) => setKabulSozlesme(e.target.checked)} style={{ marginTop: 2 }} />
                    📄 Kullanıcı Sözleşmesi'ni okudum, kabul ediyorum.
                  </label>
                  <label style={{ display: "flex", alignItems: "flex-start", gap: 10, cursor: "pointer", fontSize: 12, color: "#5a6a7e" }}>
                    <input type="checkbox" checked={kabulZirh} onChange={(e) => setKabulZirh(e.target.checked)} style={{ marginTop: 2 }} />
                    🛡️ Siber Kalkan: ödeme teslimat sonrası aktarılır. Onaylıyorum.
                  </label>
                </div>
