...
export default function SiberBorsaPaneli() {
  ...
  // 🛡️ SİBER GÜVENLİK: ŞİFRE DEĞİŞTİRME MOTORU
  const handleSifreDegistir = async (e: React.FormEvent) => {
    ...
    if (sifreForm.yeni.length < 6) return alert("Güvenlik Zafiyeti: Şifre en az 6 karakter olmalıdır!");
    ...
    try {
      const res = await fetch("/api/guvenlik/sifre-degistir", {
        ...
        body: JSON.stringify({ 
          ...
          eskiSifre: sifreForm.eski, 
          yeniSifre: sifreForm.yeni 
        })
      });
      ...
      if (res.ok) {
        alert(data.message);
        setSifreForm({ eski: "", yeni: "", tekrar: "" }); // Başarılıysa formu temizle
      } else {
  ...
  const handleTeminatOde = async (takasId: string) => {
    ...
    } catch (error) { alert("Tahsilat motoruna ulaşılamadı."); }
  };
  ...
  return (
    <div className="min-h-screen bg-[#050505] py-24 px-4 text-white font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        {/* 🎛️ ANA SEKMELER (GÜVENLİK SEKMESİ EKLENDİ) */}
        <div className="flex flex-wrap md:flex-nowrap gap-4 mb-6">
          <button onClick={() => {setAktifSekme("gelen_teklifler"); setAltFiltre("hepsi");}} className={`flex-1 p-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${aktifSekme === "gelen_teklifler" ? 'bg-[#00f260] text-black shadow-[0_0_20px_rgba(0,242,96,0.2)]' : 'bg-[#0a0a0a] text-slate-400 border border-white/5 hover:bg-white/5'}`}>
            Gelen Teklifler ({gelenTakaslar.length})
          </button>
          ...
          <button onClick={() => {setAktifSekme("guvenlik"); setAltFiltre("hepsi");}} className={`w-full md:w-auto md:flex-none px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${aktifSekme === "guvenlik" ? 'bg-red-500 text-white shadow-[0_0_20px_rgba(239,68,68,0.3)]' : 'bg-[#0a0a0a] text-slate-400 border border-white/5 hover:bg-white/5'}`}>
            🛡️ Güvenlik
          </button>
        </div>

        {/* 🔍 ALT FİLTRELER (Sadece Takas Sekmelerindeyken Görünür) */}
        {aktifSekme !== "guvenlik" && (
        ...
        {/* 🛡️ SİBER GÜVENLİK İÇERİĞİ */}
        {aktifSekme === "guvenlik" && (
          <div className="bg-[#0a0a0a] border border-white/5 p-8 rounded-3xl shadow-xl max-w-2xl mx-auto animate-in fade-in zoom-in-95 duration-500 mt-10">
            <h2 className="text-2xl font-black italic uppercase mb-2 text-white">
              Siber <span className="text-red-500">Kalkan.</span>
            </h2>
            <p className="text-slate-500 text-xs mb-8">Ajan erişim şifrenizi bu terminalden güncelleyebilirsiniz.</p>
            
            <form onSubmit={handleSifreDegistir} className="space-y-4">
              <div>
                <label className="text-slate-500 text-[10px] font-black uppercase tracking-widest block mb-2">Mevcut Şifreniz</label>
                <input type="password" value={sifreForm.eski} onChange={(e) => setSifreForm({...sifreForm, eski: e.target.value})} className="w-full bg-[#030712] border border-white/10 text-white text-sm p-4 rounded-xl outline-none focus:border-red-500 transition-colors" required />
              </div>
              <div>
                <label className="text-cyan-400 text-[10px] font-black uppercase tracking-widest block mb-2">Yeni Şifreniz</label>
                <input type="password" value={sifreForm.yeni} onChange={(e) => setSifreForm({...sifreForm, yeni: e.target.value})} className="w-full bg-[#030712] border border-white/10 text-white text-sm p-4 rounded-xl outline-none focus:border-cyan-500 transition-colors" required />
              </div>
              <div>
                <label className="text-cyan-400 text-[10px] font-black uppercase tracking-widest block mb-2">Yeni Şifreniz (Tekrar)</label>
                <input type="password" value={sifreForm.tekrar} onChange={(e) => setSifreForm({...sifreForm, tekrar: e.target.value})} className="w-full bg-[#030712] border border-white/10 text-white text-sm p-4 rounded-xl outline-none focus:border-cyan-500 transition-colors" required />
              </div>
              
              <button type="submit" className="w-full mt-4 bg-red-500 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:scale-[1.02] transition-all shadow-[0_0_20px_rgba(239,68,68,0.3)]">
                ŞİFREYİ MÜHÜRLE
              </button>
            </form>
          </div>
