...
export default function SifremiUnuttum() {
  ...
  const handleKurtarma = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        // SİBER SİMÜLASYON: Normalde mail gider, biz şimdilik test için ekrana basıyoruz.
        alert(`🚨 SİMÜLASYON MODU: Normalde bu link mailinize gelirdi.\n\nLütfen Tamam'a basarak şifre yenileme ekranına gidin.`);
  ...
  return (
    <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center p-4 font-sans relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-[#0a0a0a] border border-white/5 p-8 rounded-[2.5rem] shadow-2xl relative z-10 animate-in zoom-in-95 duration-500">
        <h2 className="text-3xl font-black italic uppercase mb-2 text-white text-center">Siber <span className="text-[#00f260]">Kurtarma.</span></h2>
        ...
        <form onSubmit={handleKurtarma} className="space-y-5">
          <input 
            type="email" 
            placeholder="Ajan E-Posta Adresi" 
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            className="w-full bg-[#030712] border border-white/10 text-white text-sm p-5 rounded-2xl outline-none focus:border-[#00f260] transition-colors" 
            required 
          />
        ...
        <div className="mt-8 text-center border-t border-white/5 pt-6">
          <Link href="/giris" className="text-slate-400 hover:text-[#00f260] text-[10px] font-black uppercase tracking-widest transition-colors">
            ← TERMİNALE GERİ DÖN
          </Link>
        </div>
      </div>
