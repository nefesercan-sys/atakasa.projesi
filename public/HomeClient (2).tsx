        ...
        {videoVar ? (
          <div className="video-thumb">
            {ilkMedya.includes("res.cloudinary.com") ? (
              <img src={ilkMedya.replace(/\.(mp4|webm|mov)$/i, ".jpg")}
                className="thumb-img" alt="Video Kapak" width={520} height={220}
      ...
      <div className="card-body">
        ...
        <div className="card-actions">
          <div className="action-row-top">
            <button onClick={handleShare} className="btn-icon" aria-label="İlanı paylaş"><Share2 size={15} /></button>
            <button onClick={() => router.push(`/varlik/${ilan._id}`)} className="btn-outline flex-1">İncele</button>
            <button onClick={() => onSepeteEkle(ilan)} className="btn-cart" aria-label="Sepete ekle"><ShoppingCart size={15} /></button>
          </div>
...
export default function HomeClient({ initialIlanlar }: Props) {
  const { data: session } = useSession();
  const router = useRouter();

  // ✅ SSR'dan gelen veriyle başla — loading ekranı atlanır, LCP düşer
  const [ilanlar, setIlanlar] = useState<any[]>(initialIlanlar);
  const [loading, setLoading] = useState(false); // SSR veri varsa loading false
  ...
  // ✅ Sadece geri kalan ilanları yükle (skip=20) — ilk 20 SSR'dan geldi
  useEffect(() => {
    const kalanYukle = async () => {
      try {
        // SSR veri yoksa tamamını çek
        ...
        // Arka planda geri kalanları yükle
        setTimeout(async () => {
          try {
  ...
  const handleSepeteEkle = useCallback((ilan: any) => {
    try {
      ...
      mevcutSepet.push({ id: urunId, baslik: ilan.baslik, fiyat: Number(ilan.fiyat), resim: getImageUrl(ilan), saticiMail: ilan.satici?.email || ilan.sellerEmail || "" });
  ...
  const handleTakasGonder = async () => {
    ...
    try {
      const res = await fetch("/api/takas", {
        ...
        body: JSON.stringify({ aliciEmail: seciliIlan.satici?.email || seciliIlan.sellerEmail, hedefIlanId: seciliIlan._id, hedefIlanBaslik: seciliIlan.baslik, hedefIlanFiyat: seciliIlan.fiyat, teklifEdilenIlanId: id, teklifEdilenIlanBaslik: baslik, eklenenNakit: eklenecekNakit || 0, durum: "bekliyor" }),
      });
  ...
  const handleSiparisTamamla = async () => {
    ...
    try {
      const res = await fetch("/api/orders", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ listingId: seciliIlan._id, sellerEmail: seciliIlan.satici?.email || seciliIlan.sellerEmail, adSoyad: siparisForm.adSoyad, telefon: siparisForm.telefon, adres: siparisForm.adres, not: siparisForm.not, odemeYontemi: siparisForm.odemeYontemi, fiyat: seciliIlan.fiyat, durum: "bekliyor" }),
      });
      ...
      {videoModalUrl && (
        <div className="modal-overlay" onClick={() => setVideoModalUrl(null)} role="dialog" aria-modal="true">
          <div className="modal-box" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 640, padding: 0, overflow: "hidden" }}>
            ...
            {videoModalBaslik && (
              <div style={{ padding: "12px 16px", fontSize: 14, fontWeight: 600, color: "var(--text)" }}>{videoModalBaslik}</div>
            )}
          </div>
        </div>
      )}
